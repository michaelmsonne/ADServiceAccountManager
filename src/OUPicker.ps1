function Choose-ADOrganizationalUnit
{
	...
	(
		...
		# HIDE THE OPTION TO CREATE OU's
		...
		else
		{
			...
			else
			{
				$ADObjects = $ADsearcher.FindAll() | Where-Object{
					...
					$null -eq $_.Properties['showinadvancedviewonly'][0]
				}
			}
			...
			If ($ADObjects)
			{
				...
				$ADObjects | ForEach-Object {
					...
					{
						Add-Node -RootNode $RootNode -dname $_.properties.distinguishedname[0] -name $_.properties.name[0] -Type $Type -HasChildren $true
					}
					else
					{
						Add-Node -RootNode $RootNode -dname $_.properties.distinguishedname[0] -name $_.properties.name[0] -Type $Type -HasChildren $false
					}
				}
			}
		}
	...
	function Build-TreeView
	{
		...
		If ($DomainDN)
		{
			...
			$RootDomainNode = Add-Node -RootNode $treeNodes -dname $DomainDN  -name $DomainName -Type Domain
		}
		else
		{
			$CurrentDomain = $Forest.Domains | Where-Object{ $_.Name -eq $env:USERDNSDOMAIN }
			$Domain = $CurrentDomain.GetDirectoryEntry()
			$RootDomainNode = Add-Node -RootNode $treeNodes -dname $Domain.distinguishedName -name $CurrentDomain.Name -Type Domain
		}
	...
	$FormEvent_Load = {
		...
		elseif ($Domain)
		{
			...
			try
			{
				$DomainIP = (Test-Connection $Domain -Count 1 -ErrorAction 0).IPV4Address.IPAddressToString
	...
	$buttonOK_Click = {
		...
		{
			$Confirmation = Show-MsgBox -Prompt "Sure you will select the OU '$($Treeview.SelectedNode.Text)' (DN: $($Treeview.SelectedNode.Name))' to manage service accounts in?`r`n`r`nThis can contain normal users also so be very carefull!?" -Title "Select custom OU for Service Acconts to manage" -Icon Exclamation -BoxType "YesNo" -DefaultButton '2'
			If ($Confirmation -eq "YES")
			{
				Write-Log -Level INFO -Message "User selected a custom OU to manage serice accounts in OU '$($Treeview.SelectedNode.Text)' (DN: $($Treeview.SelectedNode.Name)) - (this can be normal users also - be carefull!) - confirmed by user"
